...
import { Plus, Trash2, Search, Snowflake, Pencil, X, ChevronLeft, ChevronRight, Scale } from 'lucide-react';
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    if (editingId) {
      const updatedProduction = production.map(p => 
        p.id === editingId 
          ? { ...p, quantityKg: parseFloat(quantity), date, observation } 
          : p
      );
              ...
              <div>
                ...
                <div className="relative">
                   <input 
                    ...
                    placeholder="Ex: 500"
                    className="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm transition-all font-bold"
                    required
                  />
