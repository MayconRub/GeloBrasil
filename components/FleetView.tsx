...
import { 
  ...
  History, Pencil, Save, X, CheckCircle2, AlertTriangle
} from 'lucide-react';
...
const VehiclesTab = ({ vehicles, onUpdate, onDelete }: any) => {
        ...
        {vehicles.map((v: Vehicle) => (
          <div key={v.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm group hover:shadow-xl transition-all relative overflow-hidden">
             <div className="flex items-center justify-between mb-8">
                <div className="w-16 h-16 bg-sky-50 text-sky-500 rounded-2xl flex items-center justify-center group-hover:bg-sky-500 group-hover:text-white transition-all">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 border-b">
            <tr><th className="px-6 py-5">Data</th><th className="px-6 py-5">Veículo</th><th className="px-6 py-5">Serviço</th><th className="px-6 py-5">KM</th><th className="px-6 py-5">Custo</th><th className="px-6 py-5">Próxima KM</th><th className="px-6 py-5 text-center">Ação</th></tr>
          </thead>
          <tbody className="divide-y text-xs font-black text-slate-700">
            {logs.map((l: MaintenanceLog) => (
              <tr key={l.id} className="hover:bg-slate-50/50">
                ...
                <td className="px-6 py-4 text-indigo-500">{l.proxima_maint_km ? `${l.proxima_maint_km.toLocaleString()} KM` : '-'}</td>
                <td className="px-6 py-4 text-center"><button onClick={() => onDelete(l.id)} className="p-2 text-slate-300 hover:text-rose-500"><Trash2 size={16}/></button></td>
              </tr>
            ))}
          </tbody>
      ...
      {isOpen && (
        <Modal title="Agendar Manutenção" onClose={() => setIsOpen(false)}>
           <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="p-5 bg-indigo-50/50 border border-indigo-100 rounded-[2rem] space-y-4">
                 <p className="text-[10px] font-black uppercase text-indigo-600 tracking-widest flex items-center gap-2"><Calendar size={12}/> Previsão Próxima Manutenção</p>
                 <div className="grid grid-cols-2 gap-4">
                    <input type="number" placeholder="KM Próxima" className="h-10 px-4 bg-white border border-indigo-100 rounded-xl text-[10px] font-black" value={form.proxima_maint_km} onChange={e => setForm({...form, proxima_maint_km: parseInt(e.target.value)})} />
                    <input type="date" className="h-10 px-4 bg-white border border-indigo-100 rounded-xl text-[10px] font-black" value={form.proxima_maint_data} onChange={e => setForm({...form, proxima_maint_data: e.target.value})} />
                 </div>
              </div>
