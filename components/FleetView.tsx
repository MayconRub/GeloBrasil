...
const FleetView: React.FC<Props> = ({ 
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 uppercase print:space-y-2 print:pb-0">
    ...
    <div className="space-y-6 print:space-y-1">
      {selectedVehicle ? (
        <div className="animate-in slide-in-from-right duration-500 space-y-6 print:space-y-2 pb-10 print:pb-0">
          ...
          <header className="bg-slate-900 text-white p-8 print:p-4 rounded-[3rem] print:rounded-lg shadow-xl print:shadow-none">
             <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:gap-1">
                <div>
                   ...
                   <h3 className="text-4xl print:text-xl font-black tracking-tighter mt-2 print:mt-0">{selectedVehicle.modelo.toUpperCase()}</h3>
                   <p className="text-[10px] print:text-[8px] font-bold text-slate-400 uppercase tracking-widest">RESUMO: {monthName}</p>
                </div>
                <div className="grid grid-cols-2 gap-4 print:gap-2">
                   <div className="bg-white/10 p-4 print:p-2 rounded-2xl print:rounded-md border border-white/10">
                      <p className="text-[8px] font-black text-sky-400 uppercase tracking-widest">MÉDIA</p>
                      <p className="text-2xl print:text-base font-black">{selectedVehicle.avgKml.toFixed(2)} KM/L</p>
                   </div>
                   <div className="bg-white/10 p-4 print:p-2 rounded-2xl print:rounded-md border border-white/10">
                      <p className="text-[8px] font-black text-sky-400 uppercase tracking-widest">TOTAL KM</p>
                      <p className="text-2xl print:text-base font-black">{selectedVehicle.totalKmPeriod.toLocaleString()} KM</p>
                   </div>
                </div>
             </div>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print:gap-2">
            <div className="lg:col-span-2 space-y-6 print:space-y-2">
              <div className="bg-white rounded-[2.5rem] print:rounded-lg border border-slate-100 print:border-slate-300 overflow-hidden">
                <div className="p-6 print:p-2 border-b border-slate-50 print:border-slate-300 flex items-center gap-2">
                    <Fuel size={20} className="text-emerald-500 print:w-4" />
                    <h4 className="text-[11px] print:text-[8px] font-black uppercase text-slate-800">Abastecimentos</h4>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 print:bg-slate-100 text-[9px] print:text-[7px] font-black uppercase text-slate-400">
                      <tr>
                        ...
                        <th className="px-6 py-4 print:px-2 print:py-1">Percorrido</th>
                        <th className="px-6 py-4 print:px-2 print:py-1">Litros</th>
                        <th className="px-6 py-4 print:px-2 print:py-1">Rendimento</th>
                        <th className="px-6 py-4 print:px-2 print:py-1 text-right">Valor</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y print:divide-slate-200 text-[10px] print:text-[7px] font-black text-slate-700">
                      {selectedVehicle.fuelEfficiency.map((l: any, idx: number) => (
                        <tr key={idx}>
                          <td className="px-6 py-4 print:px-2 print:py-1">{new Date(l.data + 'T00:00:00').toLocaleDateString()}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1">{l.km_registro.toLocaleString()}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1">{l.dist > 0 ? `${l.dist} KM` : '--'}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1">{l.litros}L</td>
                          <td className="px-6 py-4 print:px-2 print:py-1">{l.kml > 0 ? `${l.kml.toFixed(2)} KM/L` : '--'}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1 text-right text-emerald-600">R$ {l.valor_total.toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 print:bg-slate-100 text-[9px] print:text-[7px] font-black text-slate-400">
                      <tr>
                        ...
                        <th className="px-6 py-4 print:px-2 print:py-1 text-right">Custo</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y print:divide-slate-200 text-[10px] print:text-[7px] font-black">
                      {selectedVehicle.vMaint.map((m: any, idx: number) => (
                        <tr key={idx}>
                          <td className="px-6 py-4 print:px-2 print:py-1">{new Date(m.data + 'T00:00:00').toLocaleDateString()}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1 uppercase truncate max-w-[200px]">{m.servico}</td>
                          <td className="px-6 py-4 print:px-2 print:py-1 text-right text-rose-500">R$ {m.custo.toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="space-y-6 print:space-y-2">
              <div className="bg-white p-8 print:p-3 rounded-[2.5rem] print:rounded-lg border border-slate-100 print:border-slate-300">
                <h4 className="text-[11px] print:text-[8px] font-black uppercase text-slate-400 mb-6 print:mb-2 tracking-widest">Resumo Financeiro</h4>
                <div className="space-y-4 print:space-y-1">
                  <div className="flex justify-between items-end">
                    <p className="text-[9px] print:text-[7px] font-black text-slate-500">ABASTECIMENTOS</p>
                    <p className="text-lg print:text-xs font-black text-emerald-600">R$ {selectedVehicle.totalSpentFuel.toLocaleString()}</p>
                  </div>
                  <div className={`flex justify-between items-end ${selectedVehicle.totalSpentMaint === 0 ? 'print:hidden' : ''}`}>
                    <p className="text-[9px] print:text-[7px] font-black text-slate-500">OFICINA</p>
                    <p className="text-lg print:text-xs font-black text-indigo-600">R$ {selectedVehicle.totalSpentMaint.toLocaleString()}</p>
                  </div>
                  <div className="pt-4 print:pt-1 border-t border-slate-100 print:border-slate-300 flex justify-between items-end">
                    <p className="text-[10px] print:text-[8px] font-black text-slate-900 uppercase">CUSTO TOTAL</p>
                    <p className="text-2xl print:text-sm font-black text-slate-900">R$ {selectedVehicle.grandTotal.toLocaleString()}</p>
                  </div>
                </div>
              </div>

              {selectedVehicle.totalKmPeriod > 0 && (
                <div className="bg-sky-50 print:bg-white p-8 print:p-3 rounded-[2.5rem] print:rounded-lg border border-sky-100 print:border-slate-300">
                  <div className="flex items-center gap-3 mb-4 print:mb-1">
                    <Gauge size={20} className="text-sky-600 print:w-3" />
                    <h4 className="text-[11px] print:text-[8px] font-black uppercase text-sky-900">Desempenho</h4>
                  </div>
                  <p className="text-[8px] print:text-[6px] font-black text-sky-400 uppercase mb-1">Custo por KM Rodado</p>
                  <p className="text-sm print:text-[10px] font-black text-sky-900">R$ {(selectedVehicle.totalSpentFuel / selectedVehicle.totalKmPeriod).toFixed(2)} / KM</p>
                </div>
              )}
            </div>
          </div>
          ...
          {reportData.map((row: any) => (
            <div key={row.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-8 group hover:border-sky-200 transition-all">
              <div className="flex items-center gap-4 min-w-[220px]">
                <div className="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black shadow-lg">{row.placa.slice(-3).toUpperCase()}</div>
                <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 flex-1">
                 ...
                 <div className="text-right">
                    <button onClick={() => setSelectedVehicleId(row.id)} className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-sky-500 transition-all shadow-xl">Analítico</button>
                 </div>
              </div>
            </div>
          ))}
            ...
            <div key={v.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
               ...
               <div className="grid grid-cols-2 gap-3 mt-4">
                  <div className="bg-slate-50 p-4 rounded-2xl text-center">
                    <p className="text-[8px] font-black text-slate-400 uppercase">ANO</p>
                    <p className="text-sm font-black">{v.ano}</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl text-center">
                    <p className="text-[8px] font-black text-slate-400 uppercase">KM ATUAL</p>
                    <p className="text-sm font-black">{v.km_atual.toLocaleString()} KM</p>
                  </div>
               </div>
            </div>
          );
...
const FuelTab = ({ logs, vehicles, employees, onUpdate, onDelete }: any) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end no-print"><button onClick={() => setIsOpen(true)} className="bg-emerald-500 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl">ABASTECER</button></div>
      <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400">
            <tr><th className="px-6 py-5">DATA</th><th className="px-6 py-5">VEÍCULO</th><th className="px-6 py-5">LITROS</th><th className="px-6 py-5">TOTAL</th><th className="px-6 py-5 no-print">AÇÃO</th></tr>
          </thead>
          <tbody className="divide-y text-xs font-black">
            {logs.map((l: FuelLog) => (
              <tr key={l.id}>
                <td className="px-6 py-4">{new Date(l.data + 'T00:00:00').toLocaleDateString()}</td>
                <td className="px-6 py-4">{vehicles.find((v:any) => v.id === l.veiculo_id)?.placa.toUpperCase()}</td>
                <td className="px-6 py-4">{l.litros}L</td>
                <td className="px-6 py-4 text-emerald-600">R$ {l.valor_total.toLocaleString()}</td>
                <td className="px-6 py-4 no-print"><button onClick={() => onDelete(l.id)} className="text-slate-300 hover:text-rose-500"><Trash2 size={16}/></button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const MaintenanceTab = ({ logs, vehicles, employees, onUpdate, onDelete }: any) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end no-print"><button onClick={() => setIsOpen(true)} className="bg-indigo-500 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl">MANUTENÇÃO</button></div>
      <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400">
            <tr><th className="px-6 py-5">DATA</th><th className="px-6 py-5">SERVIÇO</th><th className="px-6 py-5">VALOR</th><th className="px-6 py-5 no-print">AÇÃO</th></tr>
          </thead>
          <tbody className="divide-y text-xs font-black">
...
const FinesTab = ({ logs, vehicles, employees, onUpdate, onDelete }: any) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end no-print"><button onClick={() => setIsOpen(true)} className="bg-rose-500 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl">MULTA</button></div>
      <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400">
            <tr><th className="px-6 py-5">DATA</th><th className="px-6 py-5">INFRAÇÃO</th><th className="px-6 py-5">VALOR</th><th className="px-6 py-5 no-print">AÇÃO</th></tr>
          </thead>
          <tbody className="divide-y text-xs font-black">
            {logs.map((l: FineLog) => (
              <tr key={l.id}>
                <td className="px-6 py-4">{new Date(l.data + 'T00:00:00').toLocaleDateString()}</td>
                <td className="px-6 py-4 uppercase">{l.tipo_infracao}</td>
