...
const FleetView: React.FC<Props> = ({ 
  ...
  // Auxiliar para Máscara de Moeda
  ...
  const filteredMaint = useMemo(() => (maintenanceLogs || []).filter(l => {
    const matchesDate = l.data >= startDate && l.data <= endDate;
    const vehicle = vehicles.find(v => v.id === l.veiculo_id);
    const matchesSearch = !searchTerm || (vehicle?.placa || '').toLowerCase().includes(searchTerm.toLowerCase()) || (l.servico || '').toLowerCase().includes(searchTerm.toLowerCase());
    return matchesDate && matchesSearch;
  ...
  const stats = useMemo(() => {
    ...
    vehicles.forEach(v => {
      ...
      if (allKms.length > 1) totalKmPeriod += (Math.max(...allKms) - Math.min(...allKms));
    });
    return { totalV, totalFuel, totalMaint, totalFines, oilAlerts, totalPeriod: totalFuel + totalMaint + totalFines, totalKmPeriod };
  ...
  return (
    <div className="p-4 sm:p-8 space-y-4 sm:space-y-6 animate-in fade-in duration-500 pb-20 uppercase transition-colors">
      <header className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm dark:shadow-none no-print">
        <div className="flex items-center gap-3 sm:gap-5 w-full md:w-auto">
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-3xl font-black text-slate-800 dark:text-white tracking-normal leading-none uppercase truncate">CONTROLE <span className="mx-1">DE</span> <span className="text-[#5ecce3]">FROTA</span></h2>
      ...
      <div className="px-1 sm:px-0">
        {activeTab === 'vehicles' && <VehiclesTab vehicles={vehicles} employees={employees} onUpdate={onUpdateVehicle} onUpdateMaintenance={onUpdateMaintenance} onDelete={onDeleteVehicle} maskCurrency={maskCurrency} parseCurrency={parseCurrency} />}
        {activeTab === 'fuel' && <FuelTab logs={filteredFuel} vehicles={vehicles} employees={employees} onUpdate={onUpdateFuel} onDelete={onDeleteFuel} maskCurrency={maskCurrency} parseCurrency={parseCurrency} />}
        {activeTab === 'maintenance' && <MaintenanceTab logs={filteredMaint} vehicles={vehicles} employees={employees} onUpdate={onUpdateMaintenance} onDelete={onDeleteMaintenance} maskCurrency={maskCurrency} parseCurrency={parseCurrency} />}
        {activeTab === 'fines' && <FinesTab logs={filteredFines} vehicles={vehicles} employees={employees} onUpdate={onUpdateFine} onDelete={onDeleteFine} maskCurrency={maskCurrency} parseCurrency={parseCurrency} />}
        {activeTab === 'reports' && <ReportsTab vehicles={vehicles} fuelLogs={filteredFuel} maintenanceLogs={filteredMaint} fineLogs={filteredFines} stats={stats} startDate={startDate} endDate={endDate} />}
      </div>
    </div>
  );
};
...
const VehiclesTab = ({ vehicles, employees, onUpdate, onDelete, onUpdateMaintenance, maskCurrency, parseCurrency }: any) => {
  ...
  const [oilForm, setOilForm] = useState({ veiculo_id: '', custo: '', pago: true, data: new Date().toISOString().split('T')[0], funcionario_id: '' });

  const activeEmployees = useMemo(() => employees.filter((e: Employee) => e.status === 'ATIVO'), [employees]);
  const sortedVehicles = useMemo(() => [...(vehicles || [])].sort((a, b) => ((b.km_atual || 0) - (b.km_ultima_troca || 0) >= 1000 ? 1 : 0) - ((a.km_atual || 0) - (a.km_ultima_troca || 0) >= 1000 ? 1 : 0)), [vehicles]);
          ...
          return (
            <div key={v.id} className={`bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] border ${needsOilChange ? 'border-rose-200 dark:border-rose-900' : 'border-slate-100 dark:border-slate-800'} relative overflow-hidden group transition-all`}>
               <div className="flex items-center justify-between mb-6 sm:mb-8">
                  <div className={`w-12 h-12 sm:w-16 sm:h-16 ${needsOilChange ? 'bg-rose-500 text-white shadow-xl dark:shadow-none animate-pulse' : 'bg-sky-50 dark:bg-sky-900/20 text-sky-500'} rounded-xl sm:rounded-2xl flex items-center justify-center transition-all`}>{v.tipo === 'Caminhão' ? <Truck size={24} /> : v.tipo === 'Moto' ? <Bike size={24} /> : <Car size={24} />}</div>
                  <div className="flex gap-2 no-print transition-opacity"><button onClick={() => { setForm(v); setIsOpen(true); }} className="p-2 sm:p-3 bg-slate-50 dark:bg-slate-950 text-slate-300 dark:text-slate-700 hover:text-sky-500 rounded-lg sm:rounded-xl transition-all"><Pencil size={16} /></button><button onClick={() => onDelete(v.id)} className="p-2 sm:p-3 bg-slate-50 dark:bg-slate-950 text-slate-300 dark:text-slate-700 hover:text-rose-500 rounded-lg sm:rounded-xl transition-all"><Trash2 size={16} /></button></div>
               </div>
               ...
               <div className="flex flex-wrap items-center gap-2 mt-2"><span className="inline-block font-mono text-[10px] font-black text-[#5ecce3] bg-sky-50 dark:bg-sky-950/30 px-2 sm:px-3 py-1 rounded-lg border border-sky-100 dark:border-sky-900/30 uppercase">{v.placa.toUpperCase()}</span>{assignedDriver && (<span className={`flex items-center gap-1.5 px-2 py-1 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800 text-[8px] font-black uppercase ${assignedDriver.status === 'INATIVO' ? 'text-slate-300' : 'text-slate-500 dark:text-slate-400'}`}><UserCircle size={12} className={assignedDriver.status === 'INATIVO' ? 'text-slate-200' : 'text-[#5ecce3]'} /> {assignedDriver.name} {assignedDriver.status === 'INATIVO' ? '(INATIVO)' : ''}</span>)}</div>
               <div className={`mt-6 sm:mt-8 p-4 rounded-2xl border transition-all ${needsOilChange ? 'bg-rose-50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-900/30' : 'bg-slate-50 dark:bg-slate-950 border-slate-100 dark:border-slate-800'}`}><p className={`text-[7px] sm:text-[8px] font-black uppercase tracking-widest ${needsOilChange ? 'text-rose-600' : 'text-slate-400 dark:text-slate-600'}`}>KM ÓLEO ({kmSinceOil} / 1000)</p><button onClick={() => { setOilForm({...oilForm, veiculo_id: v.id, custo: ''}); setIsOilModalOpen(true); }} className={`mt-2 w-full py-2 border rounded-lg text-[7px] sm:text-[8px] font-black uppercase transition-all no-print ${needsOilChange ? 'bg-rose-600 text-white border-rose-700' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-600 hover:text-[#5ecce3]'}`}>REGISTRAR TROCA</button></div>
            </div>
          );
        })}
      </div>
      {isOilModalOpen && <Modal title="CONFIRMAR TROCA DE ÓLEO" onClose={() => setIsOilModalOpen(false)}>
        <form onSubmit={(e) => { e.preventDefault(); onUpdateMaintenance({ id: crypto.randomUUID(), veiculo_id: oilForm.veiculo_id, funcionario_id: oilForm.funcionario_id, tipo: 'Preventiva', servico: 'TROCA DE ÓLEO', data: oilForm.data, km_registro: vehicles.find((v:any)=>v.id===oilForm.veiculo_id)?.km_atual, custo: parseCurrency(oilForm.custo), pago: oilForm.pago }); setIsOilModalOpen(false); }} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5"><label className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase ml-2 tracking-widest">Custo R$</label><input type="text" className="w-full h-12 px-5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl font-black text-xs dark:text-white outline-none" placeholder="0,00" value={oilForm.custo} onChange={e => setOilForm({...oilForm, custo: maskCurrency(e.target.value)})} required /></div>
            </div>
          </div>
...
const FuelTab = ({ logs = [], vehicles = [], employees = [], onUpdate, onDelete, maskCurrency, parseCurrency }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const [form, setForm] = useState<any>({ data: new Date().toISOString().split('T')[0], tipo_combustivel: '', km_registro: 0, litros: 0, valor_litro: '', funcionario_id: '' });
  const handleSave = (e: React.FormEvent) => { 
    e.preventDefault(); 
    const l = parseFloat(String(form.litros || 0)); 
    const v = parseCurrency(form.valor_litro); 
    onUpdate({ ...form, id: form.id || crypto.randomUUID(), valor_litro: v, valor_total: l * v, km_registro: Number(form.km_registro) || 0, tipo_combustivel: (form.tipo_combustivel || '').toUpperCase() } as FuelLog); 
    setIsOpen(false); 
  };
  return (
    <div className="space-y-4 px-1">
      <div className="flex justify-end no-print"><button onClick={() => { setForm({ data: new Date().toISOString().split('T')[0], tipo_combustivel: '', km_registro: 0, litros: 0, valor_litro: '', funcionario_id: '' }); setIsOpen(true); }} className="bg-emerald-500 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase shadow-xl">NOVA ABASTECIMENTO</button></div>
      ...
      {isOpen && <Modal title="LANÇAR ABASTECIMENTO" onClose={() => setIsOpen(false)}><form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5"><label className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase ml-2 tracking-widest">Preço R$</label><input type="text" className="w-full h-12 px-5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl font-black text-xs dark:text-white outline-none" placeholder="0,00" value={form.valor_litro} onChange={e => setForm({...form, valor_litro: maskCurrency(e.target.value)})} required /></div>
        </div>
...
const MaintenanceTab = ({ logs = [], vehicles = [], employees = [], onUpdate, onDelete, maskCurrency, parseCurrency }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const [form, setForm] = useState<any>({ tipo: 'Preventiva', data: new Date().toISOString().split('T')[0], pago: true, funcionario_id: '', custo: '', servico: '' });
  const handleSave = (e: React.FormEvent) => { 
    e.preventDefault(); 
    onUpdate({ ...form, id: form.id || crypto.randomUUID(), custo: parseCurrency(form.custo), km_registro: vehicles.find((v:any)=>v.id===form.veiculo_id)?.km_atual || 0, servico: (form.servico || '').toUpperCase() } as MaintenanceLog); 
    setIsOpen(false); 
  };
  return (
    <div className="space-y-4 px-1">
      <div className="flex justify-end no-print"><button onClick={() => { setForm({ tipo: 'Preventiva', data: new Date().toISOString().split('T')[0], pago: true, funcionario_id: '', custo: '', servico: '' }); setIsOpen(true); }} className="bg-indigo-500 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase shadow-xl">NOVA MANUTENÇÃO</button></div>
      <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
        <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 dark:bg-slate-950 text-[8px] sm:text-[9px] font-black uppercase text-slate-400 dark:text-slate-600"><tr><th className="px-4 sm:px-6 py-4 sm:py-5">DATA</th><th className="px-4 sm:px-6 py-4 sm:py-5">SERVIÇO</th><th className="px-4 sm:px-6 py-4 sm:py-5">KM</th><th className="px-4 sm:px-6 py-4 sm:py-5 text-right">VALOR</th><th className="px-4 sm:px-6 py-4 sm:py-5 no-print text-center">AÇÃO</th></tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-[10px] sm:text-xs font-black">{logs.length === 0 ? (<tr><td colSpan={5} className="py-20 text-center text-slate-300 dark:text-slate-700 uppercase tracking-widest font-black text-[10px]">Nenhum registro no intervalo</td></tr>) : logs.map((l: MaintenanceLog) => (<tr key={l.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors"><td className="px-4 sm:px-6 py-3 sm:py-4 dark:text-slate-300">{new Date(l.data + 'T00:00:00').toLocaleDateString()}</td><td className="px-4 sm:px-6 py-3 sm:py-4 uppercase dark:text-slate-300">{l.servico}</td><td className="px-4 sm:px-6 py-3 sm:py-4 dark:text-slate-400 text-[9px]">{l.km_registro} km</td><td className="px-4 sm:px-6 py-3 sm:py-4 text-rose-500 text-right">R${(l.custo || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td><td className="px-4 sm:px-6 py-3 sm:py-4 no-print text-center"><button onClick={() => onDelete(l.id)} className="text-slate-300 dark:text-slate-700 hover:text-rose-500 transition-all"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div>
      </div>
      {isOpen && <Modal title="LANÇAR MANUTENÇÃO" onClose={() => setIsOpen(false)}><form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="space-y-1.5"><label className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase ml-2 tracking-widest">Valor R$</label><input type="text" className="w-full h-12 px-5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl font-black text-xs dark:text-white outline-none" placeholder="0,00" value={form.custo} onChange={e => setForm({...form, custo: maskCurrency(e.target.value)})} required /></div>
...
const FinesTab = ({ logs = [], vehicles = [], employees = [], onUpdate, onDelete, maskCurrency, parseCurrency }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const [form, setForm] = useState<any>({ situacao: 'Em aberto', data: new Date().toISOString().split('T')[0], data_vencimento: new Date().toISOString().split('T')[0], funcionario_id: '', valor: '', tipo_infracao: '' });
  const handleSave = (e: React.FormEvent) => { 
    e.preventDefault(); 
    onUpdate({ ...form, id: form.id || crypto.randomUUID(), valor: parseCurrency(form.valor), tipo_infracao: (form.tipo_infracao || '').toUpperCase() } as FineLog); 
    setIsOpen(false); 
  };
  return (
    <div className="space-y-4 px-1">
      <div className="flex justify-end no-print"><button onClick={() => { setForm({ situacao: 'Em aberto', data: new Date().toISOString().split('T')[0], data_vencimento: new Date().toISOString().split('T')[0], funcionario_id: '', valor: '', tipo_infracao: '' }); setIsOpen(true); }} className="bg-rose-500 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase shadow-xl">REGISTRAR MULTA</button></div>
      <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-[2rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
        <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 dark:bg-slate-950 text-[8px] sm:text-[9px] font-black uppercase text-slate-400 dark:text-slate-600"><tr><th className="px-4 sm:px-6 py-4 sm:py-5">DATA</th><th className="px-4 sm:px-6 py-4 sm:py-5">INFRAÇÃO</th><th className="px-4 sm:px-6 py-4 sm:py-5 text-right">VALOR</th><th className="px-4 sm:px-6 py-4 sm:py-5 no-print text-center">AÇÃO</th></tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-[10px] sm:text-xs font-black">{logs.length === 0 ? (<tr><td colSpan={5} className="py-20 text-center text-slate-300 dark:text-slate-700 uppercase tracking-widest font-black text-[10px]">Nenhum registro no intervalo</td></tr>) : logs.map((l: FineLog) => (<tr key={l.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors"><td className="px-4 sm:px-6 py-3 sm:py-4 dark:text-slate-300">{new Date(l.data + 'T00:00:00').toLocaleDateString()}</td><td className="px-4 sm:px-6 py-3 sm:py-4 uppercase dark:text-slate-300">{l.tipo_infracao}</td><td className="px-4 sm:px-6 py-3 sm:py-4 text-rose-500 text-right">R${(l.valor || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td><td className="px-4 sm:px-6 py-3 sm:py-4 no-print text-center"><button onClick={() => onDelete(l.id)} className="text-slate-300 dark:text-slate-700 hover:text-rose-500 transition-all"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div>
      </div>
      {isOpen && <Modal title="LANÇAR MULTA" onClose={() => setIsOpen(false)}><form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="space-y-1.5"><label className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase ml-2 tracking-widest">Valor R$</label><input type="text" className="w-full h-12 px-5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl font-black text-xs dark:text-white outline-none" placeholder="0,00" value={form.valor} onChange={e => setForm({...form, valor: maskCurrency(e.target.value)})} required /></div>
...
const ReportsTab = ({ vehicles, fuelLogs, maintenanceLogs, fineLogs, stats, startDate, endDate }: any) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm"><div className="flex flex-col lg:flex-row gap-8 items-center"><div className="w-full lg:w-1/3 flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-950 rounded-[2rem] border border-slate-100 dark:border-slate-800 text-center"><div className="w-14 h-14 bg-sky-500 text-white rounded-2xl flex items-center justify-center mb-4 shadow-xl shadow-sky-200/50 dark:shadow-none"><DollarSign size={28} /></div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Custo Total da Frota</p><h3 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter">R$ {stats.totalPeriod.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3><div className="mt-4 flex items-center gap-2 px-3 py-1 bg-sky-100 dark:bg-sky-900/30 rounded-full"><Navigation size={12} className="text-sky-600 dark:text-sky-400" /><span className="text-[9px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest">{stats.totalKmPeriod.toLocaleString('pt-BR')} KM RODADOS NO PERÍODO</span></div><p className="text-[8px] font-bold text-slate-300 dark:text-slate-600 mt-4 uppercase tracking-[0.2em]">{new Date(startDate + 'T00:00:00').toLocaleDateString('pt-BR')} — {new Date(endDate + 'T00:00:00').toLocaleDateString('pt-BR')}</p></div><div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-6 w-full"><CostIndicator label="Combustível" value={stats.totalFuel} total={stats.totalPeriod} color="bg-emerald-500" icon={Fuel} /><CostIndicator label="Manutenção" value={stats.totalMaint} total={stats.totalPeriod} color="bg-indigo-500" icon={Wrench} /><CostIndicator label="Infrações" value={stats.totalFines} total={stats.totalPeriod} color="bg-rose-500" icon={AlertOctagon} /></div></div></div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col"><div className="flex items-center gap-3 mb-8"><div className="w-10 h-10 bg-slate-50 dark:bg-slate-950 rounded-xl flex items-center justify-center text-slate-400"><PieChart size={20} /></div><h4 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">Ranking de Despesas</h4></div><div className="space-y-6 flex-1 overflow-y-auto no-scrollbar max-h-[500px] pr-2">{vehicleCosts.length === 0 ? (<div className="h-full flex flex-col items-center justify-center opacity-20"><Truck size={48} /><p className="text-[10px] font-black uppercase mt-2">Sem dados</p></div>) : vehicleCosts.map((v: any, idx: number) => { const percentage = stats.totalPeriod > 0 ? (v.total / stats.totalPeriod) * 100 : 0; return (<div key={v.id} className="group"><div className="flex justify-between items-end mb-2"><div className="flex items-center gap-3"><span className="text-[10px] font-black text-slate-300">#{idx + 1}</span><div><p className="text-xs font-black text-slate-800 dark:text-slate-200 leading-none mb-1">{v.modelo}</p><div className="flex items-center gap-2"><p className="text-[8px] font-black text-sky-500 bg-sky-50 dark:bg-sky-950 px-1.5 rounded uppercase tracking-widest">{v.placa}</p>{v.kmRodado > 0 && (<span className="flex items-center gap-1 text-[7px] font-black text-emerald-500 bg-emerald-50 dark:bg-emerald-950/30 px-1.5 rounded uppercase"><Navigation size={8} /> {v.kmRodado} KM</span>)}</div></div></div><div className="text-right"><p className="text-sm font-black text-slate-900 dark:text-white leading-none">R$ {v.total.toLocaleString('pt-BR')}</p><p className="text-[8px] font-black text-slate-400 mt-1 uppercase tracking-tighter">{percentage.toFixed(1)}% do total</p></div></div><div className="h-2 bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-[#5ecce3] rounded-full transition-all duration-1000 group-hover:bg-sky-400" style={{ width: `${percentage}%` }}></div></div></div>); })}</div></div>
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col"><div className="flex items-center gap-3 mb-8"><div className="w-10 h-10 bg-slate-50 dark:bg-slate-950 rounded-xl flex items-center justify-center text-slate-400"><History size={20} /></div><h4 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">Últimas Atividades</h4></div><div className="space-y-3 flex-1 overflow-y-auto no-scrollbar max-h-[500px] pr-2">{timelineEvents.length === 0 ? (<div className="h-full flex flex-col items-center justify-center opacity-20"><Activity size={48} /><p className="text-[10px] font-black uppercase mt-2">Sem atividade no período</p></div>) : timelineEvents.map((e: any) => (<div key={e.id} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/50 hover:border-sky-100 dark:hover:border-sky-900/30 transition-all"><div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${e.bg} ${e.color}`}><e.icon size={18} /></div><div className="flex-1 min-w-0"><div className="flex justify-between items-start"><h5 className="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase truncate pr-2">{e.label}</h5><span className="text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-tighter whitespace-nowrap">{new Date(e.data + 'T00:00:00').toLocaleDateString('pt-BR')}</span></div><div className="flex justify-between items-center mt-1"><div className="flex items-center gap-2"><p className="text-[9px] font-bold text-slate-400 truncate">{vehicles.find(v => v.id === e.veiculo_id)?.placa || '---'}</p>{e.km && <span className="text-[7px] font-black text-slate-300 dark:text-slate-700">• {e.km} KM</span>}</div><p className="text-[11px] font-black text-slate-700 dark:text-slate-300">R$ {e.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div></div></div>))}</div></div>
      </div>
    </div>
  );
};
...
const CostIndicator = ({ label, value, total, color, icon: Icon }: any) => {
  ...
  return (<div className="space-y-3"><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-lg ${color} text-white flex items-center justify-center shadow-lg dark:shadow-none`}><Icon size={14} /></div><div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{label}</p><p className="text-sm font-black text-slate-800 dark:text-slate-100 leading-none">R$ {value.toLocaleString('pt-BR')}</p></div></div><div className="h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full"><div className={`h-full ${color} rounded-full`} style={{ width: `${percentage}%` }}></div></div></div>);
};

const Modal = ({ title, children, onClose }: any) => (
  <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300 no-print transition-all"><div className="absolute inset-0 bg-slate-900/60 dark:bg-black/95 backdrop-blur-sm" onClick={onClose} /><div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 shadow-2xl dark:shadow-none border border-transparent dark:border-slate-800 relative overflow-hidden animate-in zoom-in-95 duration-300"><div className="flex items-center justify-between mb-6 sm:mb-8 border-b border-slate-50 dark:border-slate-800 pb-4 sm:pb-6"><h3 className="text-base sm:text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">{title}</h3><button onClick={onClose} className="p-2 text-slate-300 dark:text-slate-700 hover:text-rose-500 transition-colors"><X size={20} className="sm:size-6" /></button></div><div className="max-h-[70vh] overflow-y-auto pr-1 no-scrollbar">{children}</div></div></div>
);
