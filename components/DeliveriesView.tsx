    ...
    <div className="p-4 sm:p-8 space-y-4 pb-24 max-w-[1600px] mx-auto overflow-x-hidden transition-colors">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
        {['TODOS', ...Object.values(DeliveryStatus)].map(status => (
          <button key={status} onClick={() => setActiveFilter(status as any)} className={`px-3 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${activeFilter === status ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white border-slate-200 dark:border-slate-700 shadow-sm' : 'bg-white dark:bg-slate-900 text-slate-300 dark:text-slate-700 border-slate-100 dark:border-slate-800 opacity-60'}`}>{status}</button>
        ))}
      </div>
            ...
            <div key={d.id} className={`bg-white dark:bg-slate-900 p-5 rounded-[1.5rem] border flex flex-col h-full ${d.status === DeliveryStatus.ENTREGUE ? 'border-emerald-100 dark:border-emerald-900/30' : 'border-slate-100 dark:border-slate-800'} shadow-sm transition-all group`}>
              <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-0.5 no-print">
                    <button onClick={() => handlePrintReceipt(d)} title="Imprimir Comprovante" className="p-1.5 text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all"><Printer size={16} /></button>
