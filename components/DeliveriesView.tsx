...
const DeliveriesView: React.FC<Props> = ({ deliveries, clients, drivers, vehicles, products, onUpdate, onDelete }) => {
  ...
  const filtered = useMemo(() => deliveries.filter(d => activeFilter === 'TODOS' || d.status === activeFilter), [deliveries, activeFilter]);
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8 pb-24 max-w-[1600px] mx-auto overflow-x-hidden transition-colors">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-slate-800 dark:text-white tracking-tighter uppercase leading-none">CONTROLE DE <span className="text-sky-500">ENTREGAS</span></h2>
        ...
        <button onClick={() => setIsOpen(true)} className="w-full sm:w-auto px-8 py-4 bg-slate-900 dark:bg-slate-100 dark:text-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl dark:shadow-none active:scale-95 transition-all">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar py-2 -mx-4 px-4 sm:mx-0 sm:px-0">
        {['TODOS', ...Object.values(DeliveryStatus)].map(status => (
          <button key={status} onClick={() => setActiveFilter(status as any)} className={`px-5 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${activeFilter === status ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 border-slate-900 dark:border-slate-100' : 'bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-600 border-slate-100 dark:border-slate-800'}`}>
            {status}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        {filtered.map(d => {
          ...
          return (
            <div key={d.id} className={`bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-[2rem] border flex flex-col h-full ${d.status === DeliveryStatus.ENTREGUE ? 'border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/5 dark:bg-emerald-950/10' : 'border-slate-100 dark:border-slate-800'} shadow-sm dark:shadow-none transition-all`}>
               <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-3">
                     <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center ${d.status === DeliveryStatus.ENTREGUE ? 'bg-emerald-500 text-white' : d.status === DeliveryStatus.EM_ROTA ? 'bg-sky-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}><Truck size={20} className="sm:size-24" /></div>
                     <div className="min-w-0">
                        <div className="flex items-center gap-2">
                           <span className={`text-[7px] font-black uppercase px-2 py-0.5 rounded ${d.status === DeliveryStatus.ENTREGUE ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-600'}`}>{d.status}</span>
                           {d.scheduledTime && <span className="flex items-center gap-1 text-[7px] font-black text-sky-500 bg-sky-50 dark:bg-sky-950/30 px-2 py-0.5 rounded"><Clock size={8} /> {d.scheduledTime}</span>}
                        </div>
                     </div>
                  </div>
                  <div className="flex gap-1 opacity-20 group-hover:opacity-100">
                     {d.status !== DeliveryStatus.ENTREGUE && <button onClick={() => updateStatus(d, DeliveryStatus.ENTREGUE)} className="p-2 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-lg"><CheckCircle2 size={16} /></button>}
                     <button onClick={() => onDelete(d.id)} className="p-2 text-slate-300 dark:text-slate-700 hover:text-rose-500"><Trash2 size={16} /></button>
                  </div>
               </div>
               ...
               <div className="flex-1 space-y-4">
                  <div className="flex items-start gap-3">
                     ...
                     <div className="min-w-0">
                        <h4 className="font-black text-slate-800 dark:text-slate-100 text-xs sm:text-sm uppercase truncate leading-tight">{client?.name || 'Cliente Removido'}</h4>
                        <div className="flex items-start gap-1.5 text-[9px] font-bold text-slate-400 dark:text-slate-600 mt-1.5 uppercase truncate"><MapPin size={10} className="shrink-0 mt-0.5" /> <span>{client?.street}, {client?.number}</span></div>
                     </div>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-800">
                    <p className="text-[7px] font-black text-slate-400 dark:text-slate-700 uppercase tracking-widest mb-3 flex items-center gap-1.5"><ShoppingBag size={10} /> Carga</p>
                    <div className="space-y-1 max-h-[100px] overflow-y-auto pr-1 custom-scrollbar">
                      {d.items && d.items.length > 0 ? d.items.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-white/60 dark:bg-slate-900/60 px-2 py-1.5 rounded-lg border border-slate-200/50 dark:border-slate-800">
                          <span className="text-[8px] font-black text-slate-600 dark:text-slate-400 uppercase truncate mr-2">{getProduct(item.productId)?.name || 'Produto'}</span>
                          <span className="text-[9px] font-black text-slate-900 dark:text-white bg-white dark:bg-slate-800 px-2 py-0.5 rounded">{item.quantity}</span>
                        </div>
                      )) : <p className="text-[8px] text-slate-300 dark:text-slate-700 font-bold uppercase text-center py-2 italic">Vazia</p>}
                    </div>
                  </div>
               </div>
               ...
               <div className="mt-4 pt-3 border-t border-slate-50 dark:border-slate-800">
                  <div className="flex items-center justify-between">
                     <div className="flex items-center gap-1.5 text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase"><Calendar size={12} /> {new Date(d.scheduledDate + 'T00:00:00').toLocaleDateString('pt-BR')}</div>
                     {d.status === DeliveryStatus.PENDENTE && <button onClick={() => updateStatus(d, DeliveryStatus.EM_ROTA)} className="px-4 py-2 bg-sky-500 text-white rounded-lg text-[8px] font-black uppercase transition-all">Liberar <ChevronRight size={12} className="inline ml-1"/></button>}
                  </div>
               </div>
            </div>
          );
