            ...
            <div key={d.id} className={`bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-[2rem] border flex flex-col h-full ${d.status === DeliveryStatus.ENTREGUE ? 'border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/5 dark:bg-emerald-950/10' : 'border-slate-100 dark:border-slate-800'} shadow-sm dark:shadow-none transition-all`}>
               <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-3">
                     {/* Fix: removed invalid sm:size prop and used className */}
                     <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center ${d.status === DeliveryStatus.ENTREGUE ? 'bg-emerald-500 text-white' : d.status === DeliveryStatus.EM_ROTA ? 'bg-sky-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}><Truck size={20} className="sm:size-6" /></div>
