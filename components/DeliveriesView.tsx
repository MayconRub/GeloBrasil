...
const DeliveriesView: React.FC<Props> = ({ deliveries, clients, drivers, vehicles, products, onUpdate, onDelete, onAddClient, settings }) => {
  ...
  // Efeito para buscar preço de tabela do cliente
  useEffect(() => {
  ...
  // Efeito para recalcular o Valor Total automaticamente quando itens mudam
  useEffect(() => {
    if (form.items && form.items.length > 0) {
      ...
      if (total > 0) {
        setLocalTotalValue(total.toLocaleString('pt-BR', { minimumFractionDigits: 2 }));
      }
    }
  }, [form.items]);
...
========================================
  ...
  const handleAddItem = () => {
    ...
    if (idx > -1) {
      ...
      if (uPrice > 0) newItems[idx].unitPrice = uPrice;
    } else {
      newItems.push({ 
