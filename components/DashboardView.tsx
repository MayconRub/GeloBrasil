    ...
    <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 max-w-[1600px] mx-auto pb-12 animate-in fade-in duration-500 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-sky-100/10 dark:shadow-none transition-all">
        <div className="flex items-center gap-5">
          <div className="relative flex items-center justify-center w-14 h-14">
            ...
            <Box 
              size={42} 
              className={`relative z-10 transition-all duration-700 drop-shadow-[0_0_12px_rgba(94,204,227,0.5)] ${metrics.isHealthy ? 'text-[#5ecce3]' : 'text-rose-400'}`} 
              strokeWidth={2}
            />
          </div>
          <div>
        ...
        <div className="flex p-1 bg-slate-100 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-inner w-full sm:w-auto">
          <button 
            ...
            className={`flex-1 sm:px-8 py-2.5 rounded-xl text-[9px] font-black tracking-widest transition-all ${period === 'daily' ? 'bg-[#5ecce3] text-white shadow-lg' : 'text-slate-400 dark:text-slate-600 hover:text-slate-600'}`}
          >
            HOJE
          </button>
          <button 
            onClick={() => setPeriod('monthly')} 
            className={`flex-1 sm:px-8 py-2.5 rounded-xl text-[9px] font-black tracking-widest transition-all ${period === 'monthly' ? 'bg-[#5ecce3] text-white shadow-lg' : 'text-slate-400 dark:text-slate-600 hover:text-slate-600'}`}
          >
            ESTE MÊS
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <MetricCard 
          ...
          color="emerald" 
          context={period === 'daily' ? 'Receita de Hoje' : 'Vendas do Mês'} 
          trend={<ArrowUpRight size={14} />} 
        />
        <MetricCard 
          label="Despesas Pagas" 
          value={`R$ ${metrics.totalExps.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} 
          icon={Receipt} 
          color="rose" 
          context={period === 'daily' ? 'Gastos de Hoje' : 'Custo Mensal'} 
          trend={<ArrowDownCircle size={14} />} 
        />
        <MetricCard 
          label="Produção" 
          ...
          icon={Snowflake} 
          color="sky" 
          context={period === 'daily' ? 'Lote Produzido' : 'Volume Mensal'} 
        />
        <MetricCard 
          label="Saldo Líquido" 
          ...
          context="Saldo Disponível" 
          isHighlight 
        />
      </div>

      {/* Analytics Chart - Full Width for better trend visibility */}
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-[3rem] border border-slate-50 dark:border-slate-800 shadow-sm dark:shadow-none relative overflow-hidden flex flex-col min-h-[450px]">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[9px] font-bold text-slate-300 dark:text-slate-600 uppercase mt-1">Comparativo de Entradas e Saídas</p>
          </div>
          <div className="flex items-center gap-4 text-[8px] font-black uppercase tracking-widest text-slate-400">
             <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-[#5ecce3]"></div> Vendas</div>
             <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-rose-400"></div> Despesas</div>
          </div>
        </div>
                  ...
                  return (
                    <div key={delivery.id} className={`p-4 rounded-2xl border transition-all ${isToday ? 'bg-amber-50/20 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/20' : 'bg-slate-50 dark:bg-slate-950/40 border-slate-100 dark:border-slate-800'}`}>
                       <div className="flex justify-between items-start mb-2">
                          ...
                          {isToday ? (
                            <span className="flex items-center gap-1 text-[8px] font-black text-amber-600 bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 rounded">
                              HOJE
                            </span>
                          ) : (
                    ...
                    <div key={expense.id} className={`p-4 rounded-2xl border transition-all ${
                      isOverdue 
                       ...
                       <div className="flex items-center justify-between mt-2">
                          <div className="text-[8px] font-bold text-slate-400 dark:text-slate-600 uppercase">
