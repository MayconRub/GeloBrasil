...
const ClientsView: React.FC<Props> = ({ clients, products, onUpdate, onDelete }) => {
  ...
  const handleSavePrices = () => {
    ...
    Object.entries(priceForm).forEach(([id, val]) => { 
      if (val && val.trim() !== '') {
        // Converte string BRL (1.234,56 ou 10,00) para number
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 pb-24 lg:pb-20 transition-colors uppercase bg-[#f8fafc] dark:bg-slate-950 min-h-screen">
      <header className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <button onClick={() => { setForm({ type: 'PARTICULAR', street: '', number: '', neighborhood: '', city: 'MONTES CLAROS' }); setIsOpen(true); }} className="w-full sm:w-auto px-6 h-12 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl font-black text-[9px] uppercase flex items-center justify-center gap-2 shadow-xl">
            ...
            <div key={c.id} className="bg-white dark:bg-slate-950 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 transition-all active:scale-[0.98]">
               <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openPriceModal(c)} className="p-2 text-slate-300 hover:text-emerald-500"><Tag size={16} /></button>
                    <button onClick={() => {setForm(c); setIsOpen(true);}} className="p-2 text-slate-300 hover:text-sky-500"><Pencil size={16} /></button>
                    <button onClick={() => { if(confirm('DESEJA EXCLUIR ESTE CLIENTE?')) onDelete(c.id); }} className="p-2 text-slate-300 hover:text-rose-500"><Trash2 size={16} /></button>
                  </div>
               </div>
               ...
               {/* Informações de Endereço Adicionadas */}
               <div className="mt-2 flex items-start gap-1.5">
                 <MapPin size={10} className="text-sky-500 shrink-0 mt-0.5" />
                 <p className="text-[7px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tight leading-tight">
                    {c.street}, {c.number} — {c.neighborhood}
                 </p>
               </div>
               
               <div className="mt-4 flex gap-2">
                 <button onClick={() => openPriceModal(c)} className="flex-1 h-9 bg-slate-50 dark:bg-slate-800 border rounded-lg text-[7px] font-black uppercase flex items-center justify-center gap-2 dark:text-white">Tabela Preços</button>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2rem] p-8 shadow-2xl relative animate-in slide-in-from-bottom duration-500">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
               <div className="space-y-1">
                 <label className="text-[8px] font-black text-slate-400 uppercase ml-2">Nome / Razão Social</label>
                 <input className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-bold text-xs uppercase dark:text-white outline-none" value={form.name || ''} onChange={e => setForm({...form, name: e.target.value.toUpperCase()})} placeholder="NOME DO CLIENTE" required />
               </div>
               <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2">WhatsApp</label>
                    <input className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-bold text-xs dark:text-white outline-none" value={form.phone || ''} onChange={e => setForm({...form, phone: e.target.value})} placeholder="(00) 00000-0000" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2">Tipo</label>
                    <select className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-black text-[10px] uppercase dark:text-white outline-none" value={form.type} onChange={e => setForm({...form, type: e.target.value as any})}>
               ...
               <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-2 space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2">Endereço (Rua)</label>
                    <input className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-bold text-xs uppercase dark:text-white outline-none" value={form.street || ''} onChange={e => setForm({...form, street: e.target.value.toUpperCase()})} placeholder="RUA / AV" required />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2">Nº</label>
                    <input className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-bold text-xs dark:text-white outline-none" value={form.number || ''} onChange={e => setForm({...form, number: e.target.value})} placeholder="000" required />
                  </div>
               </div>
               <div className="space-y-1">
                 <label className="text-[8px] font-black text-slate-400 uppercase ml-2">Bairro</label>
                 <input className="w-full h-12 px-4 bg-slate-50 dark:bg-slate-950 border rounded-xl font-bold text-xs uppercase dark:text-white outline-none" value={form.neighborhood || ''} onChange={e => setForm({...form, neighborhood: e.target.value.toUpperCase()})} placeholder="BAIRRO" required />
               </div>
