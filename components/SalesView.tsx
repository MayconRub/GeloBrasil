...
import { Sale, AppSettings, MonthlyGoal, Client, SaleItem } from '../types';
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-950 rounded-3xl border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-2 duration-300">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 mb-4">
                ...
                <div className="sm:col-span-3">
                  <input type="number" value={itemQuantity} onChange={e => setItemQuantity(e.target.value)} className="w-full h-11 px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl font-black text-center text-sm dark:text-white" />
                </div>
                <button type="button" onClick={handleAddItem} className="sm:col-span-2 h-11 bg-sky-500 text-white rounded-xl flex items-center justify-center hover:bg-sky-600 transition-all shadow-lg"><Plus size={20} /></button>
             </div>
             {items.length > 0 && (
    ...
    <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-20 max-w-[1600px] mx-auto transition-colors">
        ...
        <div className="flex items-center justify-between w-full lg:w-auto">
          <div className="flex flex-col">
            ...
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-2 flex items-center gap-2">
              <LayoutList size={14} className="text-sky-500" /> Registro Diário de Faturamento
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden min-h-[400px]">
            ...
            <div className="md:hidden divide-y divide-slate-50 dark:divide-slate-800">
                   ...
                   <div className="flex justify-between items-start">
                      <div className="flex flex-col">
                         <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-1">
                            <Calendar size={12} className="text-sky-500" /> {new Date(sale.date + 'T00:00:00').toLocaleDateString('pt-BR')}
                         </div>
                         <h4 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase leading-snug">{sale.description}</h4>
                      </div>
                      <div className="flex gap-2 shrink-0">
                         <button onClick={() => handleEdit(sale)} className="p-3 bg-slate-50 dark:bg-slate-950 text-slate-400 dark:text-slate-600 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm active:scale-90 transition-all"><Pencil size={16} /></button>
                         <button onClick={() => handleDelete(sale)} className="p-3 bg-rose-50 dark:bg-rose-950/30 text-rose-400 dark:text-rose-900 rounded-xl border border-rose-100 dark:border-rose-900/20 shadow-sm active:scale-90 transition-all"><Trash2 size={16} /></button>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between mt-auto">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-1.5">
                           <span className="max-w-[120px] truncate text-[9px] font-black text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/30 px-2 py-1 rounded-lg border border-sky-100 dark:border-sky-900/30 uppercase">
                             {getClientName(sale.clientId)}
                           </span>
                        </div>
                        {sale.items && <span className="text-[7px] font-black text-sky-400 uppercase flex items-center gap-1"><Package size={8}/> Venda Detalhada</span>}
                      </div>
                      <div className="text-lg font-black text-emerald-600 dark:text-emerald-500 tracking-tighter">
              ...
              {filteredSales.length === 0 && (
                <div className="py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">Nenhuma venda encontrada</div>
              )}
            </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left table-fixed">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  ...
                  {filteredSales.length === 0 && (
                    <tr><td colSpan={5} className="py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">Nenhuma venda encontrada no intervalo</td></tr>
                  )}
                </tbody>
              </table>
            </div>
