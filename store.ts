...
export const syncFuel = async (f: FuelLog) => {
  ...
  if (!error) {
    ...
    await supabase.from('despesas').insert({ id: crypto.randomUUID(), descricao: `ABASTECIMENTO (${payload.litros}L)`.toUpperCase(), valor: payload.valor_total, data_vencimento: payload.data, status: 'Pago', categoria: 'COMBUSTÍVEL', veiculo_id: f.veiculo_id, funcionario_id: f.funcionario_id, km_reading: payload.km_registro });
  }
  return { error };
};
